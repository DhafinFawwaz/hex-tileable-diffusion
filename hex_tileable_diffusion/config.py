from dataclasses import dataclass, field
from pathlib import Path

from .types import ControlNetModelId, IPAdapterModelId, IPAdapterWeightName, InpaintModelId, RollMode, SchedulerType, VaeModelId, ImageInput


@dataclass(frozen=True)
class HexWrapperConfig:

    output_size: int = 512
    hypotenuse: float | None = None
    outer_margin: float = 200.0
    inner_padding: float = 75.0
    gap_padding: float = 50.0
    feather_width: float = 30.0
    horizontal_camera_padding: float = 0.0
    vertical_camera_padding: float = 0.0
    x_offset: float = 0.0
    y_offset: float = 0.5


@dataclass(frozen=True)
class DiffusionConfig:

    model_id: InpaintModelId | str = "runwayml/stable-diffusion-inpainting"
    vae_model: VaeModelId | str | None = None
    vae_fp32: bool = True
    scheduler_type: SchedulerType | None = None

    prompt: str = ""
    negative_prompt: str = ""
    num_inference_steps: int = 50
    guidance_scale: float = 7.5
    strength: float = 0.95
    seed: int = 42
    guidance_schedule: list[float] | None = None

    use_rolling_noise: bool = True
    roll_mode: RollMode = "hex_copy"


@dataclass(frozen=True)
class ControlNetConfig:

    enabled: bool = False
    model_id: ControlNetModelId | str | None = None
    conditioning_scale: float = 0.8

    def __post_init__(self) -> None:
        if self.model_id is not None and not self.enabled:
            object.__setattr__(self, "enabled", True)


@dataclass(frozen=True)
class IPAdapterConfig:

    enabled: bool = False
    model_id: IPAdapterModelId | str | None = None
    subfolder: str = "models"
    weight_name: IPAdapterWeightName | str = "ip-adapter_sd15.bin"
    scale: float = 0.85
    reference_image_path: str | Path | None = None


@dataclass(frozen=True)
class FinetuneConfig:

    enabled: bool = False
    steps: int = 300
    lr: float = 5e-5
    crop_size: int | None = None
    mask_ratio_range: tuple[float, float] = (0.1, 0.4)
    use_lora: bool = True
    lora_rank: int = 8
    lora_alpha: int = 16
    gradient_accumulation_steps: int = 4
    use_augmentation: bool = True
    use_prompt_conditioning: bool = True
    prompt_dropout_prob: float = 0.1
    log_interval: int = 50
    cleanup_after: bool = False


@dataclass(frozen=True)
class ExteriorPassConfig:

    enabled: bool = False
    strength: float = 1.0
    steps: int = 30
    guidance_scale: float = 3.0
    seed: int | None = None


@dataclass(frozen=True)
class PostprocessConfig:

    use_latent_color_correction: bool = True
    use_pixel_pasteback: bool = True
    pasteback_feather_px: int = 12
    use_color_postprocess: bool = True


@dataclass(frozen=True)
class VisualizationConfig:

    hex_outline_thickness: int = 3
    reference_image_path: str | Path | None = None
    in_between_preview_count: int = 3


@dataclass(frozen=True)
class HexTileableDiffusionConfig:

    image_path: ImageInput
    output_path: str | Path = "."
    cache_dir: str | Path = ".cache"

    layout: HexWrapperConfig = field(default_factory=HexWrapperConfig)
    diffusion: DiffusionConfig = field(default_factory=DiffusionConfig)
    controlnet: ControlNetConfig = field(default_factory=ControlNetConfig)
    ip_adapter: IPAdapterConfig = field(default_factory=IPAdapterConfig)
    finetune: FinetuneConfig = field(default_factory=FinetuneConfig)
    exterior: ExteriorPassConfig = field(default_factory=ExteriorPassConfig)
    postprocess: PostprocessConfig = field(default_factory=PostprocessConfig)
    visualization: VisualizationConfig = field(default_factory=VisualizationConfig)
